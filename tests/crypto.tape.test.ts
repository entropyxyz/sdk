...
tape('Crypto Tests:: Parses server threshold info', async (suite) => {
  try {
  ...
  suite.equal(result.toString(), mockReturn.toString())
  suite.end()
})

tape('Crypto Tests: encrypts and signs', async (suite) => {
  const aliceSecretKey: Uint8Array = new Uint8Array([
    ...
    90, 34, 93, 151, 170, 0, 104, 45, 106, 89, 185, 91, 24, 120, 12, 16, 215, 3,
    35, 54, 232, 143, 52, 66, 180, 35, 97, 244, 166, 96, 17,
  ])
