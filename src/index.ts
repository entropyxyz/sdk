...
import { deriveNewKeysFromMnemonic, generateEntropyAccountFromSeeds, generateKeysFromMnemonic, generateKeysFromPrivateKey, getWallet } from './keys'
import SignatureRequestManager, { SigOps, SigTxOps } from './signing'
import { crypto } from './utils/crypto'
import { Adapter } from './signing/adapters/types'
import { Signer, Address, EntropyAccount} from './types'
...
export default class Entropy {
  ...
  async init (opts: EntropyOpts) {
    ...
    } else if (opts.mnemonic && opts.derivationPath) {
      this.keys = await deriveNewKeysFromMnemonic(opts.mnemonic, opts.derivationPath)
    } else if (opts.mnemonic) {
      this.keys = await generateKeysFromMnemonic(opts.mnemonic)
    } else if (opts.privateKey) {
      this.keys = await generateKeysFromPrivateKey(opts.privateKey)
    } else if (opts.sigSeed && opts.progSeed) {
      this.entropyAccount = await generateEntropyAccountFromSeeds(opts.sigSeed, opts.progSeed)
    } else {
      throw new Error('Invalid key generation options')
    }
