...
import { StashKeys, ThresholdInfo, Address } from '../../types/substrate'
...
export class SubstrateRead {
  ...
  async getThresholdInfo(stashKeys: StashKeys): Promise<ThresholdInfo> {
    ...
    for (const stashKey of stashKeys) {
        const queryResult = await this.substrate.query.stakingExtension.thresholdServers(stashKey);
        const convertedResult: AnyJson = queryResult.toHuman();

        if (Array.isArray(convertedResult) && 
            (convertedResult.length > 0 && 
            (typeof convertedResult[0] === 'string' || convertedResult[0] instanceof Uint8Array))) 
        {
            result.push(convertedResult as Address[]);
        } else {
 
            throw new Error(`Unexpected format for stash key ${stashKey}`);
        }
    }
