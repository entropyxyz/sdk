...
describe('Threshold Tests', () => {
  ...
  it(`sends key`, async () => {
    ...
    try {
      const response = await thresholdServer.sendKeys([mock], [LOCAL_SERVER])
      throw new Error(await response[0].text())
    } catch (e: any) {
      if (e == 'Error: Not Registering error: Register Onchain first') {
        assert.equal(e, 'Error: Not Registering error: Register Onchain first')
      } else {
        assert.equal(e, 'Error: Kv error: Recv Error: channel closed')
      }
    }
  })
